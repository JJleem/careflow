...
:root {
  ...
  --color-border:         #E8ECEB;
  --color-surface:        #F2F4F4;  /* 페이지 배경 — 흰 카드가 테두리 없이 분리되도록 흰색과 거리 확보 (5.3 톤 보정) */
  --color-bg:             #FFFFFF;
...
> 톤 보정(2026-07-10): 초기 구현(4px radius + 전 요소 1px 테두리)이 "공공기관처럼 정적"이라는 피드백 → 토스류 핀테크의 부드럽고 고급진 물성으로 전환. **브랜드 컬러 축은 그대로, 형태·표면 중심으로 바꾼다** (surface만 카드 분리를 위해 한 단계 진하게).

- Radius: 버튼·인풋 `12px`, 카드 `16px`, 모달·시트 `24px`, 칩·소형 요소 `8px`, 뱃지·필 `100px` — 큰 radius가 부드러움의 절반
- 표면: 페이지 배경은 `--color-surface`, 카드는 흰색 + 그림자로 **테두리 없이** 분리. 1px 테두리는 표·리스트 행 구분 등 정보 구조에만 사용
- 인풋: 테두리형 대신 **채움형** — 평시 surface 배경·무테두리, 포커스 시 흰 배경 + 민트 링. 관공서 느낌의 최대 원인이 회색 테두리 입력창이다
- 그림자: 카드 `0 1px 2px rgba(11,42,43,.04), 0 4px 16px rgba(11,42,43,.06)`, 모달 `0 12px 32px rgba(11,42,43,.16)` — 퍼지는 부드러운 그림자 두 단계만
- 간격: 4px 배수 스케일 (4/8/12/16/24/32), 카드 내부 여백 24px 기준 — 여백이 고급감을 만든다
- 인터랙션: 버튼은 press 시 `scale(0.98)` + 150ms 전환, 리스트 행은 hover 배경 — "눌리는 감각"이 살아있는 화면의 조건. `prefers-reduced-motion` 존중
...
5. 문구는 해요체 대화체 — 시스템 관점("로그인 실패") 대신 사용자 행동 관점("이메일 또는 비밀번호를 확인해 주세요"). 격식체·관공서체 금지
