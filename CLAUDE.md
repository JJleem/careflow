...
⑫ 다음 시작점: **시드 보정**(위 ⑪ 실제 스펙 반영 — service_type enum 라벨·INDICATORS·관심 제품·다건 예약 상태 분포) → E2E 스모크(Playwright devDep, 검증 스크립트를 frontend/tests 또는 e2e/로 정식화) → docker-compose에 frontend 서비스(nginx serve + /api 프록시) 추가 → README(스크린샷 다수 스크래치패드에 캡처됨: c-*, w-*, a-dashboard). 시드 라벨 바꾸면 lib/labels.ts의 SERVICE_TYPE_LABEL도 동기화할 것. 사용자는 프론트 개발자 — 백엔드 개념은 프론트 비유로. 면접노트는 `interview-notes.local.md` 누적(커밋 금지).
