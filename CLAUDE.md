...
- [ ] 예약 코어: 슬롯/예약 API, 이중 예약 차단 2중 제약(슬롯 partial unique + 고객 시간겹침 GiST EXCLUDE) + 동시성 테스트
  ...
  - [x] 슬롯 API: 상담사 개설(30분 격자 강제)/삭제(활성 예약 시 409), 고객 시간대 통합 조회 + 인근 날짜 추천 (0f8e1f0)
  - [x] 예약 생성: 하이브리드 자동 배정 + savepoint 재시도 + 사이드이펙트 원자성, 동시성 pytest 6종 통과 (e385d9a)
  - [ ] 상태 전이: 취소(슬롯 해제+대기자 알림+리마인더 삭제+브리핑 취소)/완료/노쇼 + 시점 가드
...
**직전 세션 메모 (2026-07-09)**: 백엔드 골격 완료 + 예약 코어 대부분 완료. ① 원커맨드 구동(마이그레이션→멱등 시드→서버), db 호스트 포트 5433(로컬 PG 충돌 회피) ② ERD 정합화 3건 문서 먼저 수정 ③ JWT 인증(가입 customer 고정, 로그인 실패 메시지 단일화) ④ 슬롯 API(30분 격자, 삭제 가드, 시간대 통합 조회) ⑤ 예약 생성(재상담 우선→부하 최소 배정, insert-first + savepoint 차순위 재시도, 알림·리마인더·브리핑 원자 생성, 만석 409에 대안 추천) ⑥ **동시성 pytest 6종 통과** — 테스트는 careflow_test DB(실 PG, SQLite 대체 불가)에서 barrier 스레드로 검증. 시드 계정 demo1234. **다음 작업: 상태 전이** (취소=슬롯 해제+대기자 알림+미발송 리마인더 삭제+pending 브리핑 취소, 완료/노쇼=시작 시각 이후만, 취소=시작 전만 §4.7). 사용자는 프론트 개발자 — 백엔드 개념은 프론트 비유로. 면접노트는 interview-notes.local.md 누적(커밋 금지).
